...
/*
 * This file contains the Piotr Dollar's rgbConvertMex.cpp file.
 *
 * Please copy the contents of the file rgbConvertMex.cpp to this file, after
 * the line: 
...
template<class iT> void rgb2luv_sse( iT *I, float *J, int n, float nrm ) {
  ...
  while( i<n ) {
    ...
    { // perform lookup for L and finalize computation of U and V
      ...
      for( i1=i; i1<n1; i1+=4 ) {
        _l = *(_L++);
        *(_U+1) = SUB(MUL(_l,*_U),_cminu);
        *(_V+1) = SUB(MUL(_l,*_V),_cminv);
        _U++;
        _V++;
      }
    }
...
oT* rgbConvert( iT *I, int n, int d, int flag, oT nrm ) {
  ...
  if( flag==2 && useSse )
	  rgb2luv_sse(I,(float*)J,n,(float)nrm);
